  ...
  return (
    <nav
      ...
      className="fixed top-0 left-0 right-0 z-[1000] flex items-center justify-between
                 px-[var(--gutter)] py-6 transition-all duration-700"
      role="navigation"
...
// ─── Section wrapper ────────────────────────────────────────────────────────────
const Sec = ({ id, bg = '#080808', children }: { id?: string; bg?: string; children: React.ReactNode }) => (
    ...
    <Sec id="problemes">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-white/[0.05] border border-white/[0.05] rounded-2xl overflow-hidden">
        {items.map(([num, title, text], i) => (
          <div key={num} className={`reveal delay-${i + 1} bg-[#080808] hover:bg-[#1a1a1a] transition-colors duration-400 p-10`}>
    ...
    <Sec id="services" bg="#1a1a1a">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map(({ icon, title, text, tag }, i) => (
          <div key={title} className={`reveal delay-${i + 1} service-card relative bg-[#242424] border border-white/[0.06] rounded-2xl p-8 overflow-hidden hover:border-white/[0.12] hover:-translate-y-1 transition-all duration-400 group`}>
            ...
            <span className="inline-block mt-6 text-xs font-medium tracking-wider uppercase text-[#5a5a5a] border border-white/[0.08] px-3 py-1 rounded-full">{tag}</span>
          </div>
        ))}
      </div>
    </Sec>
  ...
  return (
    <Sec id="pourquoi">
      <div className="grid lg:grid-cols-2 gap-24 items-center">
        <div className="reveal-left relative aspect-[4/5] lg:aspect-auto lg:h-[600px] rounded-3xl overflow-hidden bg-[#1a1a1a]">
          <img
        ...
        <div>
          ...
          <div className="flex flex-col gap-6">
            {points.map(({ icon, title, text }, i) => (
              <div key={title} className={`reveal delay-${i + 2} why-point flex gap-5 items-start p-5 rounded-xl hover:bg-[#1a1a1a] transition-colors`}>
    ...
    <Sec id="difference" bg="#1a1a1a">
      ...
      <div className="grid md:grid-cols-[1fr_auto_1fr] gap-8 mb-16 sm:mb-20">
        <div className="reveal-left">
          <div className="text-xs font-medium tracking-[0.15em] uppercase text-[#5a5a5a] mb-6 pb-4 border-b border-white/[0.08]">Image générée / Low-cost</div>
        ...
        <div className="reveal-right">
          ...
          <div className="flex flex-col gap-4">
            {studioItems.map(t => (
              <div key={t} className="flex gap-3 items-start">
                <Check /><span className="text-base text-[#d4d4d0] leading-normal">{t}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <Sec id="plateforme" bg="#1a1a1a">
      <div className="grid lg:grid-cols-[5fr_7fr] gap-24 items-center">
        ...
        {/* Mockup */}
        <div className="reveal-right bg-[#242424] border border-white/[0.08] rounded-3xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.5)]">
          <div className="bg-[#3a3a3a] px-5 py-3 flex items-center gap-2 border-b border-white/[0.06]">
            <div className="flex gap-1.5">
              {['#ff5f57','#febc2e','#28c840'].map(c => <div key={c} className="w-2.5 h-2.5 rounded-full" style={{ background: c }} />)}
            </div>
            <div className="flex-1 bg-[#242424] rounded px-3 py-1 text-xs text-[#7a7a7a] text-center">
          ...
          <div className="p-6 flex flex-col gap-4">
            ...
            <div>
              <div className="flex justify-between mb-1.5">
                <span className="text-xs text-[#7a7a7a]">Avancement global</span>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[['Étape', 'Modélisation'], ['Versions', 'v3 / active'], ['Livraison', 'J+3']].map(([l, v]) => (
                <div key={l} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-3">
                  <div className="text-xs text-[#7a7a7a] mb-1">{l}</div>
  ...
  return (
    <Sec id="contact" bg="#1a1a1a">
      <div className="grid lg:grid-cols-[5fr_7fr] gap-24 items-start">
        ...
        <div className="reveal-right bg-[#242424] border border-white/[0.06] rounded-3xl p-10">
          <ContactForm />
        </div>
      </div>
    </Sec>
